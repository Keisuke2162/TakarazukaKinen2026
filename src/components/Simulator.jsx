...
// 画像内の黄色いライン（内回りコースの内ラチ）に沿うように座標を取得
const PT = {
  S:         { x: 700, y: 392 },   // スタート（右下チュートの青S印）
  chuteJoin: { x: 575, y: 318 },   // チュート → 本馬場 合流点（黄ライン東端）
  G:         { x: 200, y: 318 },   // ゴール線（赤G印の直下、黄ライン上）
  bottomL:   { x: 130, y: 318 },   // 黄ライン 下ストレート左端
  topL:      { x: 130, y: 105 },   // 黄ライン 上ストレート左端
  topR:      { x: 575, y: 82 },    // 黄ライン 上ストレート右端
};
// 西カーブ（タイトな内回り）と東カーブ（やや広め）の半楕円パラメータ
const WEST = { cx: 130, cy: 211.5, rx: 25,  ry: 106.5 };
const EAST = { cx: 575, cy: 200,   rx: 110, ry: 118 };
